using System.Buffers.Binary;

namespace WYD.Network.Criptography;

public static class PacketCriptography
{
    private static readonly byte[] EncryptionKeys =
    {
        
    };
    
    public static bool Decrypt(Span<byte> packet)
    {
        var size = BinaryPrimitives.ReadInt16LittleEndian(packet);
        if (size < PacketConstants.MinimumPacketSize || size != packet.Length)
        {
            return false;
        }
        
        byte keyword = packet[2];
        byte checkSum = packet[3];

        byte checkSumEnc = 0;
        byte checkSumDec = 0;

        int keywordIncrement = EncryptionKeys[keyword * 2];

        for (int i = 4; i < size; i++, keywordIncrement++)
        {
            checkSumEnc += packet[i];

            byte keywordValue = EncryptionKeys[(keywordIncrement % 256) * 2 + 1];

            switch (i & 3)
            {
                case 0:
                    packet[i] -= (byte)(keywordValue << 1);
                    break;
                case 1:
                    packet[i] += (byte)(keywordValue >> 3);
                    break;
                case 2:
                    packet[i] -= (byte)(keywordValue << 2);
                    break;
                case 3:
                    packet[i] += (byte)(keywordValue >> 5);
                    break;
            }

            checkSumDec += packet[i];
        }

        return checkSum == (byte)(checkSumEnc - checkSumDec);
    }

    public static void Encrypt(Span<byte> packet, IKeywordGenerator keywordGenerator)
    {
        var packetSize = (short)packet.Length;
        
        BinaryPrimitives.WriteInt16LittleEndian(packet, packetSize);

        var keyword = keywordGenerator.Generate();
        packet[2] = keyword;
        
        // TODO: Write current timestamp value

        byte checkSumEnc = 0;
        byte checkSumDec = 0;

        int keywordIncrement = EncryptionKeys[keyword * 2];
        
        for (int i = 4; i < packetSize; i++, keywordIncrement++)
        {
            checkSumDec += packet[i];
            
            byte keywordValue = EncryptionKeys[(keywordIncrement % 256) * 2 + 1];
            
            switch (i & 3)
            {
                case 0:
                    packet[i] += (byte)(keywordValue << 1);
                    break;
                case 1:
                    packet[i] -= (byte)(keywordValue >> 3);
                    break;
                case 2:
                    packet[i] += (byte)(keywordValue << 2);
                    break;
                case 3:
                    packet[i] -= (byte)(keywordValue >> 5);
                    break;
            }
            
            checkSumEnc += packet[i];
        }

        packet[3] = (byte)(checkSumEnc - checkSumDec);
    }
}